...
def launch_setup(context, *args, **kwargs):
    config_path = LaunchConfiguration('config_path', default= \
        os.path.join(get_package_share_directory("fast_lio_sam_sc_qn"), "config"))
    ...
    fast_lio_sam_sc_qn_params = params["fast_lio_sam_sc_qn_node"]["ros__parameters"]
    fast_lio_sam_sc_qn_params['result']['save_map_path'] = context.perform_substitution(save_map_path)

    fast_lio_sam_sc_qn_node = Node(
    ...
    timer_action = TimerAction(
        ...
        actions=[fast_lio_sam_sc_qn_node]
    )
